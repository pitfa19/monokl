...
description: >
  ...
  within their source_budget. Invoked via Jcode skill loader from the entry
...
1. **Spawn K `monokl-depth-investigator` subagents in parallel** (ONE message, all swarm tasks). One per locus with `source_budget > 0`, capped at 6.
...
4. **Read the interim notes.** After all return, list them:
   ```bash
   hyperresearch note list --tag <vault_tag> --type interim --all --json
   ```
   Then batch-read them:
   ```bash
   hyperresearch note show <id1> <id2> ... -j
   ```
...
```
skill_manage load name="monokl-6-cross-locus-reconcile")
```
