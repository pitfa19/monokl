...
description: >
  ...
  Invoked via Jcode skill loader from the entry skill (full tier only).
---
...
- All interim notes: `hyperresearch note list --tag <vault_tag> --type interim --all --json` then `hyperresearch note show <id1> <id2> ... -j`
...
```
skill_manage load name="monokl-7-source-tensions")
```
