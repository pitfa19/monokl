...
description: >
  ...
  and source count gating. Invoked via Jcode skill loader from the entry skill
...
**Scores travel with the URLs.** When you assign batches (step 2.4), include each URL's composite utility score next to it. Fetchers pass it to `hyperresearch fetch --utility-score <N>` so the score persists into note frontmatter — it becomes one input to the vault's composite `quality_score`, which step 10 uses for ranked curation.
...
```bash
PYTHONIOENCODING=utf-8 hyperresearch note list --tag <vault_tag> --all --json | python -c "import sys,json; d=json.load(sys.stdin); print(f'Notes in vault: {len(d.get(\"data\",[]))}')"
```
...
1. **List fetched sources:** `hyperresearch note list --tag <vault_tag> --all --json` — count substantive (non-deprecated) notes.
...
```bash
hyperresearch claims ingest --tag <vault_tag> -j          # claims JSONs -> queryable claims table
hyperresearch sources backfill-doi --tag <vault_tag> -j   # catch DOIs the fetchers missed
hyperresearch sources score --tag <vault_tag> -j          # citation counts + retraction check (cached APIs)
hyperresearch graph rank -j                               # vault centrality + composite quality_score
```
...
```bash
hyperresearch escalation list --status queued --tag <vault_tag> -j
```

**If queued items exist**, spawn EXACTLY ONE `monokl-browser-fetcher` subagent to drain them (serial, one browser — never spawn two):
...
prompt: |
  ...
  YOUR INPUTS:
  - vault_tag: <vault_tag>
  - drain up to 10 items (claim via `hyperresearch escalation claim --tag <vault_tag>`)
...
2. In non-interactive (`-p`) runs where no user can answer: record `hyperresearch run block <vault_tag> --on human-challenges -j` and CONTINUE the pipeline with everything else — the queue drains on the next `hpr run resume`.
3. After the user says done: `hyperresearch escalation retry <id>` each item, re-spawn the browser-fetcher once, then re-run step 2.7's ranking commands so the new sources are scored.
...
When a single long source (>5000 words) is load-bearing, delegate end-to-end analysis to `monokl-source-analyst` (full-source deep read):

Trigger conditions (ALL three must hold):
1. **Length:** source's `word_count` (visible on `hyperresearch note show <id> -j`) exceeds ~5000 words
...
- **light tier:** Skip directly to step 10 — invoke `skill_manage load name="monokl-10-triple-draft")` (light tier writes a single draft, not the ensemble)
- **full tier:** Invoke `skill_manage load name="monokl-3-contradiction-graph")`
