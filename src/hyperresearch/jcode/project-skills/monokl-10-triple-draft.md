...
description: >
  ...
  skips ahead to step 15 (polish). Invoked via Jcode skill loader.
---
...
**⚠ CRITICAL ANTI-PATTERN: Writing a single draft for `full` tier is a PIPELINE VIOLATION.** In V7 runs, context compaction caused the orchestrator to forget this step's procedure and write a single draft instead of spawning 3 sub-orchestrators. V8 fixes this by loading this skill fresh at the moment it's needed. **If you find yourself about to write `research/notes/final_report_<vault_tag>.md` directly without spawning 3 `monokl-draft-orchestrator` subagents, STOP. Re-read this skill. Spawn the sub-orchestrators.** (Light tier is the ONE exception — see "Light tier" section below.)
...
- Survey vault: `hyperresearch note list --tag <vault_tag> --all -j` for the evidence landscape
...
1. **Read the vault directly.** Light tier has no `evidence-digest.md` (step 9 was skipped). Survey the vault: `hyperresearch note list --tag <vault_tag> --all -j` and pick the 8–15 most relevant non-deprecated notes. Read each one (`hyperresearch note show <id1> <id2> ... -j`) before writing.
...
5. **Exit and route.** Once `research/notes/final_report_<vault_tag>.md` is written, return to the entry skill and invoke `skill_manage load name="monokl-15-polish")`. Light tier skips steps 11–14 entirely.
...
1. **List all substantive vault notes:**
   ```bash
   hyperresearch note list --tag <vault_tag> --all --json
   ```
   ...
   **Rank the pool before picking.** For each atomic item, run a quality-ranked search to surface the best-evidence sources first:
   ```bash
   hyperresearch search "<atomic item keywords>" --tag <vault_tag> --ranked -j
   ```
...
**Spawn 3 `monokl-draft-orchestrator` subagents in ONE message.** This is true parallel execution. Each gets a different `draft_id`, `analytical_angle`, and (CRUCIALLY) a different `must_read_note_ids` array.
...
- **light tier:** You already wrote `research/notes/final_report_<vault_tag>.md` directly. Skip steps 11-14 (no synthesis, no critics, no patcher) and invoke `skill_manage load name="monokl-15-polish")`.
- **full tier:** Invoke `skill_manage load name="monokl-11-synthesize")`.
